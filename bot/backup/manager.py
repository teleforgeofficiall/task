...
class BackupManager:
    ...
    def _get_pg_env(self) -> tuple[dict, dict]:
    ...
    async def restore_backup(self, filepath: str) -> dict:
        """
        Restore a PostgreSQL backup from a custom-format .sql.gz file.
        ...
        try:
            ...
            stdout, stderr = await process.communicate(input=dump_data)

            if process.returncode != 0:
                raise BackupError(f"pg_restore failed: {stderr.decode(errors='replace')}")
